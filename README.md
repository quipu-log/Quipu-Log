...
Logs who did what, to which entity, through which API. Each log pins the entity versions current when it was written, so old logs stay searchable by the values that were true at the time.

## What it does

- **Tamper-evident.** Append-only storage on an RFC 6962 Merkle history tree. `verify_integrity()` catches in-place edits and swapped segments. Inclusion/consistency proofs are O(log n), checkable without the full log or trust in the operator.
- **Versioned entities.** Actors and targets are typed, versioned records. A renamed entity is still found by its old name.
- **No database.** The store is a directory over `std::fs`. No daemon, no dependency; a crash recovers by truncating the torn tail. Cost: you manage encryption keys ([Key management](#key-management)).
- **Non-blocking writes.** Async pipeline: retries, disk-backed DLQ, fallback hook.
