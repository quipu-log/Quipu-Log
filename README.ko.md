...
### 다른 스토리지 환경 예상 처리량 (모델 추정 — 측정 아님)

위 수치는 Apple 노트북 기준입니다. 이 환경의 `F_FULLFSYNC`는 정직하게 durable하지만 느립니다(약 12ms). group commit 덕분에 `EveryN(N)` 처리량은 fsync에 묶입니다 — 한 배치 비용 = `N × 이벤트당 CPU + fsync 1회`. 측정 baseline에서 이벤트당 CPU ≈ 18µs(64건 배치당 약 1.14ms), M4 fsync ≈ 12ms. 아래 표는 CPU를 고정하고 대표적인 서버 스토리지의 durable 쓰기 지연(Linux `fdatasync`)만 대입한 것입니다. **위 모델에서 계산한 추정치이지 벤치마크가 아니므로**, 대상 환경에서 실측하기 전까지는 자릿수 수준으로만 보세요.

| 스토리지 (가정 fsync) | 단일 트리 `EveryN(64)` 예상 기록 처리량 |
|---|---|
| 엔터프라이즈 로컬 NVMe, 전원손실보호(PLP) (약 0.08ms) | 약 52,000 events/s |
| 클라우드 고IOPS 블록, 예: io2 Block Express (약 0.5ms) | 약 39,000 events/s |
| 데이터센터 SATA SSD (약 1.0ms) | 약 30,000 events/s |
| 클라우드 범용 SSD, 예: gp3 / pd-ssd (약 1.5ms) | 약 24,000 events/s |
| 처리량형 HDD / 콜드 블록, 예: st1 (약 8ms) | 약 7,000 events/s |

`OsManaged`는 페이지 캐시에 묶이므로 위 환경 전부에서 CPU 천장(약 56,000 events/s) 근처에 머뭅니다. fsync 지연이 배치의 CPU 비용보다 작아지면 `EveryN`도 같은 천장으로 수렴합니다 — 빠른 NVMe 행이 천장 바로 아래에 있는 이유입니다.

### single writer 너머로 확장: 테넌트 샤딩

Merkle 트리는 writer가 하나라 위 수치는 트리당 천장입니다. 더 올리려면 독립 트리 N개를 두고 테넌트를 각 트리로 라우팅합니다. `quipu-middleware`의 `ShardRouter`가 이걸 해줍니다 — 테넌트 기준 consistent-hash 라우팅, 샤드별 group commit 병렬 실행, 그리고 서명된 cross-shard anchor로 무결성(tamper-evidence)이 모든 샤드에 걸쳐 유지됩니다. 처리량은 공유 디바이스의 fsync 대역이 포화될 때까지 약 N× 로 늘어납니다(PLP NVMe면 그 천장이 훨씬 멀어집니다). 단, 샤딩은 테넌트 *간* 확장이지 테넌트 *내* 확장이 아닙니다 — 핫 테넌트 하나는 여전히 트리 하나에 떨어집니다.
...
**고른 분산은 호출자 책임입니다.** 라우터는 넘긴 tenant 키를 consistent-hash 할 뿐, 부하를 보고 재배치하지 않습니다 — "가장 한가한 샤드로 보내기" 같은 동작은 없습니다. 이건 동적 로드밸런싱이 아니라 **애플리케이션 레벨 파티셔닝**입니다. 고른 분포는 *키 설계*에서 나옵니다: 테넌트당 키 하나면 테넌트가 (볼륨이 아니라 개수 기준으로) 고르게 퍼지고, `tenant:substream`처럼 더 잘게 쪼갠 키면 핫 테넌트 한 명의 쓰기도 여러 샤드로 분산됩니다(대신 순서 보장은 substream 단위로 좁아짐). tenant를 정하는 쪽 — 보통 raw 클라이언트 입력이 아니라 요청 신원에서 도출하는 신뢰된 인증 계층 — 이 그 선택을 책임집니다.

`n`은 쓰기 동시성에 맞춰 정합니다(코어당 샤드 1개 정도가 무난한 출발점). 시드는 고정·영속화해야 합니다 — 링이 시드에 의존하므로, 재시작 후에는 `ShardMap::from_parts(active, frozen, seed)`로 복원합니다. 재배치(rebalance)는 샤드를 `freeze` 하면 됩니다 — 쓰기는 멈추되 읽기는 계속되며, 이것이 single-writer-for-life 보장을 지키는 방식입니다. `ShardMap::new(1, seed)`는 샤드 1개짜리 degenerate 케이스로 샤딩 안 한 스토어와 완전히 동일하게 동작하므로, `n = 1`로 시작해 나중에 키워도 됩니다.
