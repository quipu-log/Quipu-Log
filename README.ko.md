...
데몬이 하나의 프로세스인 것은 빠진 기능이 아니라 설계 결정입니다.
단일 writer라서 스토어가 루트를 파일 락 하나로 지키고 테이블마다 끊기지 않는 해시 체인 하나를 유지할 수 있습니다 —
다른 무엇도 쓰지 않기 때문에 위변조 탐지가 단순해집니다.
다중 노드 복제는 그 단순함을 대가로 가용성을 삽니다.
writer가 둘이면 체인 헤드에 대한 합의 프로토콜이든, 갈라진 체인을 맞추는 절차든 필요하고,
둘 중 하나에 버그가 생기면 "로그가 변조되지 않았음을 증명한다"가 "아마 변조되지 않았을 것이다"로 무너집니다.
우리는 증명을 지키고, 가용성은 싸게 풀리는 곳 — 클라이언트 — 에서 해결하는 쪽을 택합니다.

단일 노드의 대가는 솔직히 적습니다: **데몬이 죽었거나 쓰기 큐가 가득 차 있는 동안 모든 호출 서비스의 감사 기록이 멈춥니다.**
감사 로그에서는 일반 데이터보다 이게 더 중합니다 — 누락된 기록은 새로고침하면 되는 게 아니라 컴플라이언스 구멍이니까요.
그래서 장애를 건너 기록을 지키는 책임을 송신 측으로 옮기고, 기댈 수 있는 서버 측 보장 두 가지를 둡니다:

- **멱등 키.** `POST /v1/logs`는 `Idempotency-Key` 헤더를 받아 최근 받은 키들을 기억하므로,
  확인하지 못한 요청을 클라이언트가 재시도해도 두 번째 기록이 생기지 않습니다.
- **클라이언트가 찍는 발생 시각.** 이벤트는 자기 `occurred_at`을 싣고, 로그는 그것을 그대로 보존합니다.
  재시도 후, 혹은 클라이언트 디스크 버퍼에서 재생돼 몇 분 늦게 도착한 이벤트도 *행위가 일어난 시각*을 기록하므로,
  지연이 최종 로그에 드러나지 않습니다.

`quipu-client`가 이 계약의 송신 측 레퍼런스 구현입니다:
멱등 재전송, 지터를 곁들인 지수 백오프, 그리고 장애를 견디다 데몬이 돌아오면 재생하는 옵트인 로컬 디스크 스풀.
HTTP 의존성은 없고 — 메서드 하나짜리 트레잇으로 쓰던 HTTP 라이브러리에 끼웁니다.
자세한 내용은 [README](crates/quipu-client/README.ko.md)를 보세요.

계획된 다운타임이나 노드 장애에는 라이브 페일오버가 아니라 **콜드 스탠바이**로 복구합니다:
봉인된 세그먼트는 불변이라 언제든 안전하게 복사되고,
액티브 세그먼트와 레지스트리 꼬리만 복사 전에 `flush`(또는 정상 종료)하면 됩니다.
복사한 루트를 가지고 스탠바이 호스트에서 데몬을 띄우면 —
파일 락을 잡고, 찢어진 꼬리를 잘라내고, 다시 쓰기 가능해집니다.
재시작은 스토어 전체가 아니라 *액티브* 세그먼트 크기에 비례합니다.
봉인된 세그먼트는 다시 스캔되지 않으니까요.
클라이언트는 그 공백 동안 이벤트를 스풀에 들고 있습니다.
단계별 절차는 [quipu-server README](crates/quipu-server/README.ko.md#콜드-스탠바이-페일오버)에 있습니다.
...
- **익스포트** — `POST /v1/logs/export`는 쿼리 결과를 줄바꿈 구분 JSON(`application/x-ndjson`)으로
  덤프합니다. 감사인 인계와 SIEM 적재용입니다. 전체 스캔이니 시간 범위로 묶으세요.
- **SIEM 포워딩** — 서버 `sink` 설정 섹션으로 모든 기록 이벤트를 syslog(RFC 5424/UDP)로 미러링할 수
  있습니다. 베스트에포트이고 쓰기 경로를 막지 않습니다. 다른 전송은 `quipu_middleware::SinkFn`으로
  끼웁니다.

### 라이브 스토어 백업

깔끔한 백업은 한 성질에 달려 있습니다: **봉인된 세그먼트는 불변이다.** 세그먼트가 가득 차 다음으로
넘어가면 옛 파일은 다시 바뀌지 않으므로, 데몬이 도는 중에도 안전하게 복사됩니다 — 증분 도구(`rsync`,
스냅샷 diff)는 새로 봉인된 세그먼트와 자라는 액티브 꼬리만 옮깁니다.

일관 시점 복사를 원하면 액티브 꼬리를 먼저 안정시키세요:

1. `POST /v1/admin/flush`(또는 데몬 정상 종료)로 액티브 세그먼트와 레지스트리 꼬리를 fsync합니다.
   봉인된 세그먼트는 이미 최종이라 flush가 필요 없습니다.
2. 스토어 루트 전체를 복사합니다. flush 없이 쓰기 중간에 뜬 복사본도 *복구는 됩니다* — open 때 마지막
   온전한 CRC 유효 레코드에서 찢어진 꼬리를 잘라냅니다 — 다만 flush가 있어야 복사본이 직전까지 확인된
   이벤트를 정확히 담습니다.
3. 복원한 복사본을 `store.verify_integrity()`(또는 `POST /v1/admin/verify`)로 검증합니다. 모든 해시
   체인을 따라가므로, 세그먼트를 잃었거나 순서가 뒤바뀐 복사본은 신뢰되지 않고 잡힙니다.

보존 정책은 봉인 세그먼트를 `unlink`로 통째 지웁니다. 삭제가 아니라 아카이브가 필요하면 보존 윈도가
지나기 **전에** 세그먼트를 복사해 두세요 — 스토어는 퇴출 시 아카이브하지 않습니다.
