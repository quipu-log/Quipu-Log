...
누가 어떤 API로 무엇에 무슨 일을 했는지 기록합니다. 각 로그는 기록 시점에 최신이던 엔티티 버전을 가리켜 두므로, 옛 로그는 그때 참이었던 값으로 그대로 검색됩니다.

## 무엇을 하나

- **변조 감지.** RFC 6962 Merkle history tree 위의 append-only 저장. `verify_integrity()`가 제자리 수정과 바꿔치기된 세그먼트를 잡습니다. inclusion/consistency proof는 O(log n)이며, 전체 로그나 운영자 신뢰 없이 검증됩니다.
- **버전 관리 엔티티.** actor와 target은 타입이 있는 버전 관리 레코드입니다. 이름이 바뀐 엔티티도 옛 이름으로 찾힙니다.
- **데이터베이스 없음.** 스토어는 `std::fs` 위 디렉토리 하나. 데몬도 의존성도 없고, 크래시는 쓰다 만 꼬리를 잘라 복구합니다. 대가는 키 관리입니다([키 관리](#키-관리)).
- **논블로킹 쓰기.** 비동기 파이프라인: 재시도, 디스크 기반 DLQ, 폴백 훅.
