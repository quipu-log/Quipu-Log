...
- **`Ngram(n)` 인덱스가 있으면:** 토큰 다이제스트로 매칭(대소문자 무시, 평문은 디스크에 없음). 쿼리의 모든 n-gram이 레코드에 있는지 보는 *후보* 필터라, 같은 조각들이 흩어져 있으면(전체 부분문자열이 없어도) 과매칭이 날 수 있습니다. 필드가 `Rsa` *보호*면 후보를 복호화해 실제 값과 재대조하므로(`plaintext_cache(true)` 필요) 히트가 정확하지만, `Sha256`/`Hmac` 필드는 되살릴 평문이 없어 재대조를 못 해 그 과매칭이 거짓 양성으로 남습니다. `contains()`를 정확히 하려면 그 필드를 `Rsa` 보호로 거세요(`Rsa`는 매칭 연산자가 아니라 필드 보호 종류입니다).
