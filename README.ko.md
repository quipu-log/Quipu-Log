...
- **`Ngram(n)` 인덱스가 있으면:** 토큰 다이제스트로 매칭(대소문자 무시, 평문은 디스크에 없음). 쿼리의 모든 n-gram이 레코드에 있는지 보는 *후보* 필터라, 조각들이 순서 없이 흩어져 있으면(전체 부분문자열이 없어도) 과매칭이 날 수 있습니다. 이 과매칭이 걸러지는지는 필드의 *보호*(연산자가 아니라)에 달려 있습니다:
    - **`Rsa`:** 후보를 복호화해 실제 값과 재대조(`plaintext_cache(true)` 필요) → 정확한 히트.
    - **`Sha256` / `Hmac`:** 되살릴 평문이 없어 재대조 불가 → 과매칭이 거짓 양성으로 남음.
